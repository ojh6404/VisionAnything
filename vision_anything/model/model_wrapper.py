...
from vision_anything.model.model_base import InferenceModelBase
from vision_anything.config.config_base import InferenceConfigBase
from vision_anything.utils.vis_utils import (
    overlay_davis,
    nhw_to_hw,
    hw_to_nhw,
    BOX_ANNOTATOR,
...
class DEVAModel(InferenceModelBase):
    ...
    def predict(self, image, sam_model, detection_model):
        with torch.cuda.amp.autocast(enabled=self.model_config.amp):
            ...
            if segments_info:
                ...
                for seg in segments_info:
                    ...
                    labels.append(
                        f"{detection_model.classes[seg['category_id']]} {seg_id_to_mask_id[seg['id']]}"
                    )
        ...
        return detections, segmentation, visualization
    ...
    def predict(self, image: np.ndarray):
        ...
        if "panoptic_seg" in predictions:  # when panoptic segmentation
            ...
            if "instances" in predictions:
                ...
                if classes:
                    xyxy = instances.pred_boxes.tensor.numpy()  # [N, 4]
...
class CutieModel(InferenceModelBase):
    ...
    def set_model(
        ...
        if labels is not None and classes is not None:
            self.obj_id_to_class_id = {i: labels[i] for i in range(len(labels))}
            self.classes = classes
        else:
            self.obj_id_to_class_id = None
            self.classes = None
    ...
    def predict(self, image: np.ndarray):
        with torch.cuda.amp.autocast(enabled=True):
            ...
            if self.obj_id_to_class_id is not None and self.classes is not None:
                obj_id = np.unique(self.segmentation)[1:]  # exclude background
                class_id = [self.obj_id_to_class_id[id - 1] for id in obj_id]
                masks = []
                for id in obj_id:
                    mask = self.segmentation == id
                    masks.append(mask)
                ...
                visualization = LABEL_ANNOTATOR.annotate(
                    scene=visualization,
                    detections=detections,
                    labels=[f"{self.classes[id]} {object_id}" for id, object_id in zip(class_id, obj_id)],
                )
...
class MASAModel(InferenceModelBase):
    ...
    def predict(self, image: np.ndarray, detection_model: Optional[InferenceModelBase] = None):
        ...
        if detection_model is not None and self.model_config.model_type == "masa_r50":
            ...
            det_bboxes = torch.tensor(detections.xyxy)
            det_scores = torch.tensor(detections.confidence)
            det_bboxes = torch.cat([det_bboxes, det_scores.unsqueeze(1)], dim=1)
            det_labels = torch.tensor(detections.class_id)
            ...
            labels_with_instances = [
                f"{detection_model.classes[label_id]} {instance_id}"
